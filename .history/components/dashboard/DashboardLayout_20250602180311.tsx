        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="hidden sm:flex items-center space-x-3">
            ...
            <div className="hidden md:block">
              <p className="text-sm font-medium">Thomas Dupont</p>
              <p className="text-xs text-muted-foreground">Étudiant • L2 Informatique</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <aside className="hidden md:block w-64 glass border-r border-white/20 p-4">
          <nav className="space-y-2">
            {sidebarItems.map((item, i) =>
              item.divider ? (
                <div key={i} className="my-4 border-t border-white/10" />
              ) : (
                <Link
                  key={i}
                  href={item.href || "/"}
